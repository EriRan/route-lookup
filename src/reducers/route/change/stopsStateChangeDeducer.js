...
/**
 * Handles changes that happen to startStop, destinationStop and calculatedRoute states when it is not immediately known did the start or the destination stop change
 * @param {*} currentState
 * @param {*} payload
 */
export function changeStartOrDestination(currentState, payload) {
  if (payload.hasError) {
    return currentState;
  }
 ...
 */
export function appendCalculatedRoute(currentState) {
  if (
